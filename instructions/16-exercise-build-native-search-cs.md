---
lab:
  title: Build Native Search Over a Product Catalog in C#
...
# Build Native Search Over a Product Catalog in C#
...
1. Create a project and open its folder in the integrated terminal.
    ...
    ```powershell
    dotnet new console --name search-lab
    cd search-lab
    ```
...
1. Replace the contents of **Program.cs** with the following code. Replace the two placeholder values with your endpoints.
    ...
    for (int start = 0; start < products.Count; start += 50)
    {
        ...
        List<string> texts = batch.Select(BuildSearchText).ToList();
        ...
        for (int i = 0; i < batch.Count; i++)
        {
            ...
            batch[i]["contentHash"] = ContentHash(texts[i]);
    ...
    static string BuildSearchText(JObject product) =>
...
1. Replace the contents of **Program.cs** with the following code, keeping the same project and packages and replacing the two placeholder values with your endpoints.
    ...
    string searchText = "something to protect my head";
    ...
    while (feed.HasMoreResults)
    {
        ...
        {
            Console.WriteLine($"  {item.similarityScore:F4}  {item.name}");
        }
...
1. Replace the contents of **Program.cs** with the following code, keeping the same project and packages and replacing the two placeholder values with your endpoints.
    ...
    FeedIterator<JObject> changeFeed = container.GetChangeFeedIterator<JObject>(
        ChangeFeedStartFrom.Beginning(), ChangeFeedMode.LatestVersion);

    while (changeFeed.HasMoreResults)
    {
        FeedResponse<JObject> page = await changeFeed.ReadNextAsync();

        if (page.StatusCode == HttpStatusCode.NotModified) { break; }

        foreach (JObject change in page)
        {
            scanned++;
            string text = BuildSearchText(change);

            if (ContentHash(text) == change["contentHash"]?.ToString()) { continue; }

            OpenAIEmbedding embedding = await embeddingClient.GenerateEmbeddingAsync(text);

            change["searchText"] = text;
            change["contentHash"] = ContentHash(text);
            change["embedding"] = JArray.FromObject(embedding.ToFloats().ToArray());

            await container.UpsertItemAsync(
                change, new PartitionKey(change["categoryId"]!.ToString()));

            refreshed++;
            Console.WriteLine($"Refreshed: {change["name"]}");
        }
    }

    Console.WriteLine($"Scanned {scanned} changes, refreshed {refreshed}.");
