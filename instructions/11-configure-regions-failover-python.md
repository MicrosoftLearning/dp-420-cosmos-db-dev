---
lab:
  title: Configure Regions and Failover in Python
...
1. Replace the contents of the program file with the following code, then set the endpoint and the preferred region to the values you recorded.
    ...
    PREFERRED_REGION = "<your-second-region>"
    ...
    client = CosmosClient(
        ...
        preferred_locations=[PREFERRED_REGION],
    )
    ...
    verified = container.read_item(item=ITEM_ID, partition_key=CATEGORY_ID)
    print(f"Read back {verified['name']}.")
    ```
