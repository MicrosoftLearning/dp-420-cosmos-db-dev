---
lab:
  title: Diagnose throttling and configure an alert in C#
...
The .NET client disables automatic retries on rate limiting. This setting makes throttling easier to observe in the exercise; it isn't the default production retry setting.

1. Open a new integrated terminal in **Visual Studio Code** and create a console project.
...
1. Add the packages the project needs. `Microsoft.Azure.Cosmos` requires an explicit `Newtonsoft.Json` reference, and without it the project doesn't build.
...
1. Replace the contents of **Program.cs** with the following code, then replace `<cosmos-endpoint>` with the endpoint you recorded earlier.
    ...
    using (FeedIterator<dynamic> iterator = container.GetItemQueryIterator<dynamic>("SELECT * FROM c"))
    {
    ...
    Console.WriteLine($"Read {products.Count} products.");

    int succeeded = 0;
    int throttled = 0;
    double charge = 0;
    object chargeLock = new();
        ...
        await Task.WhenAll(products.Select(async product =>
        {
            try
            {
                ItemResponse<dynamic> response = await container.UpsertItemAsync<dynamic>(
                    product,
                    new PartitionKey((string)product.categoryId));

                Interlocked.Increment(ref succeeded);
                lock (chargeLock) { charge += response.RequestCharge; }
            }
            catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.TooManyRequests)
            {
                Interlocked.Increment(ref throttled);
                lastActivityId = ex.ActivityId;
            }
        }));
    }

    Console.WriteLine($"Succeeded: {succeeded}");
    Console.WriteLine($"Rate limited (429): {throttled}");
    Console.WriteLine($"Request units consumed: {charge:0}");
    Console.WriteLine($"Activity ID of a rate-limited request: {lastActivityId}");
    ```

1. Run the application. It takes about a minute plus the time to read the container.
    ...
    Read 295 products.
    Succeeded: 3184
    Rate limited (429): 9260
    Request units consumed: 22947
...
1. Select **Apply splitting**, choose the `StatusCode` dimension, and set the time range to the last hour.
    ...
    The two numbers are close for the same workload window because you disabled automatic 429 retries. With the default retries, some rate-limited attempts succeed on retry and never become application errors. Rerunning this timed workload can also change the request volume and the service's 429 count, so compare each run within its own time window.
